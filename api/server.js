...
app.post("/daftar", async (req, res) => {
  const { full_name, email, password, age, role } = req.body;
  try {
    const post_data = await db.query(`INSERT INTO user(full_name, email, password, age, role, is_verified) 
                                     VALUES ("${full_name}", "${email}", "${password}", "${age}", "${role}", FALSE)`);

    if (post_data) {
      const user = { id: post_data.insertId, full_name, email, age, role };
